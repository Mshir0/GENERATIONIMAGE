...
`white_spot` 默认在整张目标图的有效边界内均匀随机放置，不读取颜色、花型或纹理内容。不同记录使用不同 `seed` 即可在同一张正常图上得到不同且可复现的位置。如需限制位置，也可以将 `placement.mode` 改为 `dark_regions`，或在 `parameters.mask.placement.mask_path` 中提供人工允许区域 mask。
